...
namespace SE_Mod_ADP_Reworked
{
    ...
    [ProtoContract]
    public class MessageToggleStoneEnable : MessageBase
    {
        [ProtoMember(1)]
        public long EntityId;

        [ProtoMember(2)]
        public bool EnableStone;
        ...
        private void DoWork()
        {
            ...
            if (MyAPIGateway.Entities.TryGetEntityById(EntityId, out block))
            {
                block.GameLogic.GetAs<Rig>().StoneEnabled = EnableStone;
                (block as IMyAssembler).StoreTerminalValues();
            }
        }
    }
    ...
    [XmlInclude(typeof(MessageToggleStoneEnable))]
